...
class Environment(object):
    ...
    hard_time_limit = -100000  # even if enforce_deadline is False, end trial when deadline reaches this value (to avoid deadlocks)
    ...
    def __init__(self, num_dummies=3):
        ...
        self.trial_data = {
            ...
            'negative_reward': 0.0,  # total reward earned in current trial
    ...
    def reset(self):
        ...
        deadline = self.compute_dist(start, destination) * 5*0
    ...
    def act(self, agent, action):
        ...
        if agent is self.primary_agent:
            ...
            self.trial_data['final_deadline'] = - state['deadline']
            self.trial_data['net_reward'] += reward
            
            if reward < 0:
                self.trial_data['negative_reward'] += reward
...
#~        col_dist=  abs (b[0] - a[0])  
#~        row_dist=  abs (b[1] - a[1])  
#~            
#~        return(  min( col_dist, abs( self.grid_size[0] - col_dist )   ) +  min( row_dist,  abs( self.grid_size[1] - row_dist ) )   )
